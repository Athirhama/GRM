...
class DGCNN_PartSeg(nn.Module):
    def __init__(self, k=20, num_parts=50, num_categories=16):
        ...
        # Global aggregation
        self.conv4 = nn.Sequential(nn.Conv1d(192, 1024, kernel_size=1, bias=False),
                                   nn.BatchNorm1d(1024), nn.LeakyReLU(0.2))
        
        # Category label branch
        self.label_conv = nn.Sequential(nn.Conv1d(num_categories, 64, kernel_size=1, bias=False),
                                        nn.BatchNorm1d(64), nn.LeakyReLU(0.2))

        # Segmentation head
        self.conv5 = nn.Sequential(nn.Conv1d(1280, 256, kernel_size=1, bias=False),
